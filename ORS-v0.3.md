...
| `scope_snapshot` | object | Yes | Compact record of the scope axes the recorded action claims to have consumed. The snapshot is an assertion by the receipt issuer about consumed scope; it is not a proof of compliance on its own. Checking scope containment requires pairing the snapshot with the retrieved and verified mandate commitment, which carries the allowed scope, the principal, and the validity window — none of which appear in the receipt — per the verification procedure defined below and authoritatively in the W1 commitment specification, Section 7. Its members are `action` (string, the action type performed), `domain` (string, the domain interacted with), and `spend` (object with `currency` and an integer `amount` in minor units), each present only if the action consumed that axis. A snapshot with no consumed axes is the empty object `{}`. |
...
*Interoperability note (non-normative).* For public OpenTerms Mandate deployments interoperating across organizations, the following profile is suggested as guidance: an issuer SHOULD emit UUID version 7 `mandate_id` values, and a verifier SHOULD accept any non-empty string as a valid `mandate_id` unless the verifier is operating under a stricter local policy. This profile keeps issuance disciplined (so chronological sorting and registry lookups behave predictably across deployments) while keeping the public verification surface tolerant (so a strict issuer and a permissive verifier interoperate by default, and a strict verifier can opt in via local policy). This is guidance only; it does not change the normative `mandate_id` rule above, which remains a string with UUID version 7 RECOMMENDED, not required.
...
**Strict mode — relying parties that require a mandate.** A relying party operating under a policy that requires a mandate — for example, a registry whose `mandate_policy.required` is `true`, an API route gated on delegated authority, or any caller expecting the `mandate-bound-and-verified` posture defined in Section 8 — MUST call a mandate-bound verifier that performs the full Section 11.1 / W1 Section 7 procedure. It MUST NOT call only the Section 8 receipt verifier and accept its `{valid: true}` result. Being a cryptographically valid ORS receipt is not sufficient where a mandate is required: an unbound receipt, or a bound receipt whose binding has not been verified, MUST be a verification failure under strict mode. The Section 8 receipt verifier and a mandate-bound verifier are not interchangeable.

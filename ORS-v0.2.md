...
* Added optional `terms_type`, `terms_service`, and `terms_version` payload fields for policy classification. All are optional and signed when present. Existing v0.1 receipts remain valid.
* Added `model_training` to the `action_type` enum.
* Promoted `ors.anchor` and `ors.zk_proof` extension namespaces from informational stubs to normative descriptions with verification semantics.
* Added Appendix C: Migration guide from v0.1 to v0.2.
...
| `ors_version` | string | No | ORS format version identifier. If present, value SHOULD be `0.2`. |
...
A compatible JSON-LD context for ORS receipts MAY be published at `https://openterms.io/ns/ors/v0.2/context.jsonld`. This is informational and not required for conformance.
